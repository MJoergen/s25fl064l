-------------------------------------------------------------------------------
--  File name : testbench_s25fl064l_vhdl.vhd
-------------------------------------------------------------------------------
...
-- ENTITY DECLARATION
-------------------------------------------------------------------------------
ENTITY testbench_s25fl064l_vhdl IS

END testbench_s25fl064l_vhdl;
-------------------------------------------------------------------------------
-- ARCHITECTURE DECLARATION
-------------------------------------------------------------------------------
ARCHITECTURE vhdl_behavioral_static_memory_allocation
                                              of testbench_s25fl064l_vhdl IS
    ...
    FOR ALL: s25fl064l USE ENTITY work.s25fl064l;
    ---------------------------------------------------------------------------
    ...
    --model configuration
    ---------------------------------------------------------------------------
    CONSTANT mem_file         : string  := "s25fl064l.mem";
    CONSTANT secr_file        : string  := "s25fl064lSECR.mem";
    CONSTANT half_period1_srl : time    := 10 ns;   --1/(2*50MHz)
    CONSTANT half_period2_srl : time    := 4.6 ns;  --1/(2*108MHz)
    CONSTANT half_period_ddr  : time    := 9.25 ns; --1/(2*54MHz)

    CONSTANT UserPreload      : boolean := TRUE;
    CONSTANT LongTimming      : boolean := TRUE;
    CONSTANT TimingModel      : STRING  := "S25FL064LABMFI000_15pF";
    ...
    SHARED  VARIABLE half_period   : TIME    := half_period2_srl;--4.6 ns
    SHARED  VARIABLE ManufID_DevID : ManufDevID_type := (16#01#, 16#60#, 16#17#);
    SHARED  VARIABLE DevID : DevID_type := (16#17#, 16#60#);
    ...
    BEGIN
        ...
        GENERIC MAP (
            ...
            --with any port.
            -- WRR Cycle Time
            tdevice_WRR                => 1200 ms,  --tW
            -- Page Program Operation
            tdevice_PP                 => 1350 us,  --tPP

            -- Byte Programming
            tdevice_BP1                => 90 us,  --tBP1
            -- Byte Programming
            tdevice_BP2                => 30 us,  --tBP2

            -- 4 KB Sector Erase Operation
            tdevice_SE                 => 320 ms, --tSE
            -- 32 KB Half Block Erase Operation
            tdevice_HBE                => 600 ms, --tHBE
            -- 64 KB Block Erase Operation
            tdevice_BE                 => 1150 ms, --tBE

            -- Chip Erase Operation
            tdevice_CE                 => 150 sec, --tCE
            -- Suspend latency
        ...
        ---------------------------------------------------------------------------
            ...
            XOn               => DefaultXon,
            -- memory file to be loaded
            mem_file_name     => "s25fl064l.mem",
            secr_file_name    => "s25fl064lSECR.mem",
            ...
            -- For FMF SDF technology file usage
            TimingModel       => "S25FL064LABMFI000_15pF"
        )
        PORT MAP(
            SCK        => T_SCK,
            SI         => T_SI,
...
--------------------------------------------------------------------------
   ...
   ----------------------------------------------------------------------------
    ...
    PROCEDURE bus_cycle(
        ...
        sector    :IN   INTEGER RANGE 0 TO SecNum+1  := 0;
    ...
    BEGIN
        ...
        CASE bus_cmd IS
            ...
            WHEN bus_desel_read    =>
                IF Clock_polarity = '1' THEN
                    WAIT UNTIL rising_edge(T_SCK);
                    IF half_period = half_period2_srl THEN
                        WAIT FOR 5 ns;
                    ELSE
                        WAIT FOR 5 ns;
                    END IF;
   ...
   ----------------------------------------------------------------------------
    ...
    IS
        ...
        VARIABLE addr_tmp     : NATURAL RANGE 0 TO AddrRANGE_REG;
        VARIABLE SECTOR_REG   : NATURAL RANGE 0 TO SecNum+1;
    ...
    BEGIN
        ...
        CASE command.cmd IS
            ...
            WHEN    wrar         =>
                ...
                SECTOR_REG := command.sect;
                ADDR       := command.addr;
                addr_tmp   := SECTOR_REG*(SecSize+1) + ADDR;
...
END PROCESS checker;
    ...
    BEGIN
        ...
        SFDP_array(16#0326#) := 16#00#;
        SFDP_array(16#0327#) := 16#EE#;
